...
export async function generateAndSendOtp(email: string): Promise<{ ok: boolean; message: string }> {
  ...
  await prisma.verificationToken.create({
    data: { identifier: email, tokenHash, expiresAt: new Date(Date.now() + 5 * 60_000) },
  });
...
export async function verifyOtpAndLogin(email: string, otp: string): Promise<{ ok: boolean; message: string; userId?: string; sessionToken?: string; expiresAt?: Date }> {
  ...
  const token = await prisma.verificationToken.findFirst({
    where: { identifier: email, tokenHash, used: false, expiresAt: { gte: new Date() } },
  ...
  if (!user) {
    user = await prisma.user.create({
      data: { email, emailVerified: new Date(), nickname: email.split('@')[0], authProvider: 'email', status: 'active', lastLoginAt: new Date() },
    });
