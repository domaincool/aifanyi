...
export default function PdfTranslatorPage() {
  ...
  const cancelTask = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        setJob((prev: any) => (prev ? { ...prev, status: 'cancelled', message: '任务已取消。' } : prev));
      } else {
  ...
  const resumeTask = useCallback(async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setError({ errorType: 'insufficient', message: data.message || data.error || '任务未能恢复，请稍后重试。' });
      }
    } catch {
    ...
    <div className="pdf-page">
      ...
      <section className="pdf-seo">
            ...
            "mainEntity": [
              {
                ...
                "acceptedAnswer": {
                  "@type": "Answer",
                  "text": "当前为免费体验阶段。新用户注册送 500 积分，按页数计积分（每页 20 积分）；游客每日可先试 5 个文件 / 50 页，注册后升至 10 个文件 / 100 页，每日自动重置。"
                }
              },
              {
                "@type": "Question",
        ...
        <div className="pdf-seo-faq">
          <div className="pdf-seo-faq-item"><h3>免费吗？有次数限制吗？</h3><p>当前为免费体验阶段。新用户注册送 500 积分，按页数计积分（每页 20 积分）；游客每日可先试 5 个文件 / 50 页，注册后升至 10 个文件 / 100 页，每日自动重置。</p></div>
        ...
        <div className="pdf-summary">
          ...
          {job && (job.status === 'queued' || job.status === 'processing') && (
            <div className="pdf-progress">
              <button className="pdf-btn" style={{ marginBottom: 8 }} onClick={cancelTask}>✕ 取消任务</button>
            ...
            <div className="pdf-done">
              ...
              {job.result?.stats && (
                <p className="pdf-done-stats">
                  耗时 {Math.round((job.result.stats.durationMs || 0) / 1000)}s
          ...
          {job && job.status === 'paused' && (
            <div className="pdf-summary" style={{ border: '1px solid #f0a020' }}>
              <h2>⏸️ 任务已暂停</h2>
              <p style={{ margin: '8px 0' }}>{job.message || '任务已暂停，请稍后重试。'}</p>
              <p style={{ margin: '8px 0', fontSize: 14 }}>点击下方按钮继续翻译，无需重新上传。</p>
              <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                <a className="pdf-btn" href="/account">查看用量</a>
