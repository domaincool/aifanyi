...
> 版本：P1 完整交付（阶段 1-9）｜提交：7771c82 → 3c500fc（errorType 修复）→ c988ab4（stats.failedBlocks 口径修复）→ 6b462f3（双栏解析修复）｜日期：2026-08-10
...
### 4.3 样本补测（公开样本，6b462f3）
| 样本 | 类型 | 结果 |
|---|---|---|
| pdf.js encrypted-attachment | 加密 | ✅ errorType=encrypted（422） |
| pdf.js images（纯图片） | 扫描版 | ✅ errorType=no_text_layer（422） |
| arXiv 1706.03762（15 页双栏） | 双栏论文 | ✅ 检出多栏 limitation + 全链路翻译完成（90/90 可翻译块，$0.014/91s） |
| 损坏文件 | corrupt | ✅ errorType=corrupt |
| 单栏 test-doc | 单栏回归 | ✅ 不误报（dual=false） |
| multicol（3 栏） | 多栏 | 保守不判双栏（已知限制：仅 2 栏检出） |
| 超限（>100 页/20MB/100 万字符） | 限额 | 校验逻辑代码级验证（上传即 throw） |

**补测发现的 bug（已修复，6b462f3）**：双栏 PDF 行聚类按 y 归行（±2px），左右栏同行被拼成一行 → 块级 x 全部取左栏起点 → 双栏检测永不命中、翻译顺序错乱。修复：Pass 1 收集 item 级归一化 x 做「双峰 + 栏间空白（0.45-0.5 无 item）」检测，行聚类按中线拆左/右栏行。回归：加密/扫描/损坏/单栏均不受影响。
