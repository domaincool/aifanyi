...
const SRC = 'C:/Users/Administrator/.openclaw-autoclaw/agents/agent-p6qd/workspace/projects/aifanyi/research/langpair-v2-batchI-20260924.json';
const OUT = path.join(TMP, 'langpair-v2I-full.json');
const PROG = path.join(TMP, 'langpair-v2I-progress.json');
...
const CFG = JSON.parse(fs.readFileSync(path.join(TMP, 'v2i-config.json'), 'utf8'));
...
function hasTerm(text, term) {
  ...
  if (words.length >= 2) {
    const rx = (lead) => words.map((w, i) => (i ? '(?: [a-z]+){0,2} ' : lead) + w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?:ing|ed|es|s|d)?').join('');
...
const SLUG_OVERRIDE = { '彩蛋': 'post-credits-scene' };
