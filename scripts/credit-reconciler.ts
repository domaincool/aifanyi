...
import { writeDailySnapshot } from './reconcile-snapshot';
...
async function main() {
  ...
  // V1.2 P1-A-4b：每日对账全量快照（计数/合计/当日新增落盘 JSON，失败不阻塞主流程）
  await writeDailySnapshot({
    ...
    console.error('[credit-reconciler] snapshot failed:', e?.message || e);
  });
}
