...
import { DeepSeekProvider } from '../src/lib/translator/providers/deepseek';
import { GlmProvider } from '../src/lib/translator/providers/glm';
import { GoogleTranslateProvider } from '../src/lib/translator/providers/google';

// ---- 手动加载 .env（独立 tsx 不加载 .env） ----
const envPath = '/opt/aifanyi/.env';
if (fs.existsSync(envPath)) {
  for (const line of fs.readFileSync(envPath, 'utf8').split('\n')) {
    const m = line.match(/^([A-Z0-9_]+)=(.*)$/);
    if (m && !process.env[m[1]]) process.env[m[1]] = m[2].trim().replace(/^"(.*)"$/, "$1").replace(/^\x27(.*)\x27$/, "$1");
  }
}

const SOURCES: { category: string; text: string }[] = [
  // 1. general 普通
...
async function main() {
  ...
  const items: any[] = [];
  let ok = 0, fail = 0;
  const progressPath = '/opt/aifanyi/.openclaw-tmp/blindtest-progress.jsonl';
  if (fs.existsSync(progressPath)) fs.unlinkSync(progressPath);

  for (let i = 0; i < SOURCES.length; i++) {
    const src = SOURCES[i];
    const req = { text: src.text, sourceLang: 'en', targetLang: 'zh', scenario: 'pdf' } as const;
    const withTimeout = (p: Promise<any>, ms: number, tag: string) =>
      Promise.race([p, new Promise<any>((res) => setTimeout(() => res({ text: '', model: tag, error: 'timeout' }), ms))]);
  ...
  const outPath = '/opt/aifanyi/pdf-blindtest-50-20260810.json';
