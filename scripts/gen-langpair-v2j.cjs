...
const SRC = 'C:/Users/Administrator/.openclaw-autoclaw/agents/agent-p6qd/workspace/projects/aifanyi/research/langpair-v2-batchJ-20260926.json';
const OUT = path.join(TMP, 'langpair-v2J-full.json');
...
const words = [];
for (const c of data.categories) for (const p of c.pairs) words.push({ zh: Array.isArray(p) ? p[0] : p.zh, en: Array.isArray(p) ? p[1] : p.en, cat: c.name });
...
const CFG = JSON.parse(fs.readFileSync(path.join(TMP, 'v2j-config.json'), 'utf8'));
...
function buildFixPrompt(w, prev, issues) {
  return [
    ...
    '',
    '词条对：中文「' + w.zh + '」⇄ 英文「' + w.en + '」，分类「' + w.cat + '」。',
    '\u3010\u786c\u6027\u7eaa\u5f8b\u3011examples\u7684\u4e2d\u6587\u7ffb\u8bd1\uff08zh \u5b57\u6bb5\uff09\u91cc\u7981\u6b62\u51fa\u73b0\u4efb\u4f55\u82f1\u6587\u5b57\u6bcd\uff08App\u2192\u5e94\u7528\u3001SIM\u2192\u7535\u8bdd\u5361\u3001iOS\u2192\u82f9\u679c\u7cfb\u7edf\uff09\uff1b\u4e24\u65b9\u5411\u7684 shortAnswer \u5fc5\u987b\u540c\u65f6\u5305\u542b\u4e2d\u6587\u8bcd\u4e0e\u82f1\u6587\u8bcd\u539f\u5f62\uff1b\u6240\u6709 shortAnswer 40-100 \u5b57\u7b26\u3001definition 45-140 \u5b57\u7b26\uff0c\u5b81\u77ed\u52ff\u8d85\u3002',
...
function hasTerm(text, term) {
  ...
  const PLACEHOLDER_WORDS = new Set(['someone', 'somebody', 'one', 'oneself', 'sth', 'something']);
  const phraseWords = words.filter((w) => !PLACEHOLDER_WORDS.has(w));
  const useWords = phraseWords.length >= 1 ? phraseWords : words;
  if (words.length >= 2) {
    const rx = (lead) => useWords.map((w, i) => (i ? '(?: [a-z]+){0,2} ' : lead) + w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?:[bdgklmnprt])?(?:ing|ed|es|s|d)?').join('');
    if (new RegExp('(^|[^a-z])' + rx('') + '([^a-z]|$)').test(t)) return true;
    if (new RegExp('(^|[^a-z])' + rx('(?: (?:that|the|a|an|it|them))?') + '([^a-z]|$)').test(t)) return true;
  }
  const ir = IRREG[head];
...
(async () => {
  ...
  async function worker() {
    while (idx < words.length) {
      const w = words[idx++];
      const key = w.zh + '|' + w.en;
      // WORKER_GUARD: never let one bad word kill the run
      try {
      ...
      for (let att = 1; att <= 3 && !out; att++) {
        try {
          ...
          const o1 = parseJson(txt);
          if (o1 && o1.zh && o1.en && Array.isArray(o1.examples)) { out = o1; lastIssues = validate(w, out); }
          else { lastIssues = ['首轮输出结构不完整']; out = null; }
        } catch (err) {
      ...
      let fixRounds = 0;
      while (out && lastIssues.length && fixRounds < 4) {
        fixRounds++;
        try {
          const txt = await callDS([{ role: 'user', content: buildFixPrompt(w, out, lastIssues) }], 3000);
...
const zSlug = SLUG_OVERRIDE[w.zh] || slugZh(w.zh), eSlug = slugEn(w.en);
      ...
      } catch (err) { fail++; console.log('GUARD_CATCH ' + (w ? (w.zh + '|' + w.en) : '?') + ' :: ' + String(err.message || err).slice(0, 100)); }
    }
  }
