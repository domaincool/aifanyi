...
exports.detectSourceLang = detectSourceLang;
...
async function parsePdf(data, fileName) {
    const started = Date.now();
    let pdf;
    try {
        pdf = await (0, pdf_js_1.getDocument)({ data: new Uint8Array(data), isEvalSupported: false }).promise;
    }
    catch (e) {
        console.error('[pdf-parse] getDocument error:', e?.name, e?.message?.slice(0, 200));
        if (e?.name === 'PasswordException') {
            throw new types_1.PdfError('encrypted', '该 PDF 已加密，需要密码才能打开。请先解除密码保护后再上传。');
        }
        throw new types_1.PdfError('corrupt', '文件损坏或不是有效的 PDF，无法解析。请检查文件后重试。');
    }
    if (pdf.numPages > config_1.PDF_CONFIG.maxPages) {
        throw new types_1.PdfError('too_many_pages', `该 PDF 共 ${pdf.numPages} 页，超过单文件 ${config_1.PDF_CONFIG.maxPages} 页的上限。请拆分后上传。`);
    }
    const pages = [];
    ...
    if (allItemXs.length > 20) {
        ...
        isDualColumn = left > allItemXs.length * 0.25 && right > allItemXs.length * 0.25 && Math.abs(left - right) < allItemXs.length * 0.3;
    }
    const splitX = 0.475;
    for (let p = 1; p <= pdf.numPages; p++) {
        let page;
        try {
            ...
            throw new types_1.PdfError('parse_failed', `第 ${p} 页解析失败，文件可能已损坏。`);
        }
        ...
        if (totalCharacters > config_1.PDF_CONFIG.maxCharacters) {
            throw new types_1.PdfError('too_many_characters', `该 PDF 文本量超过 ${(config_1.PDF_CONFIG.maxCharacters / 10000).toFixed(0)} 万字符的上限，请拆分后上传。`);
        }
        ...
        if (lines.reduce((s, l) => s + l.text.length, 0) < 50) {
            try {
                ...
                pageHasImage = ops.fnArray.some((fn) => fn >= 83 && fn <= 88); // paintImageXObject 系列
            }
            catch { /* 忽略 */ }
            if (pageHasImage)
                totalImages++;
        ...
        const flush = () => {
            ...
            if (isTopHeader)
                type = 'header';
            else if (isBottomFooter)
                type = 'footer';
            else if (fontSize >= bodyBase * 1.2 && text.length < 80)
                type = 'heading';
            else if (/^\s*([•●◦▪■□●◆◇\-*]|\d+[\.\)、]|[a-zA-Z][\.\)])/.test(text))
                type = 'list-item';
            else if (curLines.length >= 2 && curLines.every((l) => l.items >= 2 && l.text.length < 60))
                type = 'table'; // 可能表格区域
            blocks.push({
                id: `${p}-${order}`,
                type,
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (curLines.length) {
                ...
                if (gap > Math.max(prev.height, 6) * 1.3)
                    flush();
            }
            curLines.push(l);
        }
    ...
    if (totalCharacters < 20 && totalImages > 0) {
        throw new types_1.PdfError('no_text_layer', '该 PDF 是扫描版（纯图片），没有可提取的文字层，当前版本暂不支持直接翻译。请使用带文字层的 PDF（OCR 版本即将推出）。');
    }
    if (totalCharacters < 20) {
        throw new types_1.PdfError('no_text_layer', '该 PDF 没有可提取的文字内容。');
    }
    const doc = {
