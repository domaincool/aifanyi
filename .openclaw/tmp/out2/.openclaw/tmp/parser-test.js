...
(async () => {
    for (const f of files) {
        ...
        try {
            const doc = await (0, parser_1.parsePdf)(ab, f);
            const types = {};
            for (const pg of doc.pages)
                for (const b of pg.blocks)
                    types[b.type] = (types[b.type] || 0) + 1;
            console.log(`OK ${f}: ${doc.pageCount}页 sourceLang=${doc.sourceLang} 双栏=${doc.doubleColumn || false} 块=${JSON.stringify(types)}`);
        }
        catch (e) {
            if (e instanceof types_1.PdfError)
                console.log(`ERR ${f}: errorType=${e.code} | ${e.message.slice(0, 46)}`);
            else
                console.log(`ERR ${f}: ${e?.name || e?.message?.slice(0, 80)}`);
        }
    }
})();
